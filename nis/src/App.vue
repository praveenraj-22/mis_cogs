<template>
	<v-app>
		<v-navigation-drawer fixed clipped app v-model="drawer" temporary v-if="show" mini-variant
			mini-variant-width=150 class="mt-5">
			<v-list dense style="height:1000!important">
				<v-list-tile v-for="item in tabItems" :key="item" @click="changeCategory(item)"
					@click.stop="drawer = !drawer">
					<v-list-tile-action class="subheading text-xs-center font-weight-bold"
		...
		serverBus.$on("changeComponent", component => {
			...
			//finance Menu addition
				...
				if (
					...
					this.tabItems = ["AdminApproval",'competitor'];
				}
			} 
