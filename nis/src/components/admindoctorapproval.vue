        ...
        <v-flex xs12 sm10 offset-sm1 md10 offest-md1 lg10 offset-lg1>
          ...
          <template>
            <v-card-title>
              ...
              <v-spacer></v-spacer>
              <v-text-field v-model="search" v-if="findoctorlist" append-icon="mdi-magnify" label="Search" single-line
                hide-details></v-text-field>
            </v-card-title>

            <v-data-table :headers="headers" :items="findoctorlist" v-model="selected" :search="search"
              class="elevation-1">
              <template slot="items" slot-scope="props">
                <tr @click="rowClick(props.item)">
                  ...
                  <td class="text-xs-left" v-if="(props.item.Active_status==='Approved')">{{ props.item.Approved_date |
                  dateformat }}</td>
                  <td class="text-xs-left" v-if="(props.item.Active_status==='Rejected')">{{ props.item.Cancelled_time |
                  dateformat}}</td>
                  ...
                  <td class="text-xs-left" v-if="props.item.Active_status==='Pending for approval'">
                    ...
                    <v-dialog v-model="dialogcancel[props.item.ID]" persistent max-width="800px" lazy absolute
                      :key="props.item.ID">
                      <v-card>
                        <v-card-title>
                        ...
                        <v-card-text>
                          <v-container grid-list-md>
                            <v-layout wrap>
                              ...
                              <v-flex xs12 sm6>
                                <v-textarea clearable clear-icon="cancel" label="Comments" v-model='fincomments'>
                                </v-textarea>
                              </v-flex>
                            </v-layout>
                          </v-container>
                        ...
                        <v-card-actions>
                          ...
                          <v-btn color="primary" flat @click.stop="$set(dialogcancel, props.item.ID, false)">Close
                          </v-btn>
