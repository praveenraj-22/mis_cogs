<template>
      ...
      <v-layout row wrap>
        ...
        <v-flex xs12 sm6 md4>
          <v-autocomplete clearable autocomplete='off' default="" v-model="memberSelected" :items="drt"
            label="Select DRT" required item-text="Name" item-value="ID" v-on:change='getDRTdetail'></v-autocomplete>
        </v-flex>
        <v-flex xs12 sm6 md4>
        ...
        <v-flex xs12 sm6 md4>
          ...
          <input type="file" ref="referalslip" accept="image/x-png, image/gif, image/jpeg,application/pdf"
            v-on:change="handleFileUploadslip()" />
  ...
  <v-card-actions>
    ...
    <v-btn color="blue darken-1" v-model='buttonstatus' v-if="approval" flat
      @click="apiinsertbill(billid,netamount,aggcommission,drtcommission,drtamount,drtid,drtcategory,drtcomments,buttonstatus)">
      Submit</v-btn>
  ...
  methods: {
    ...
    apiinsertbill(billid, netamount, aggcommission, drtcommission, drtamount, drtid, drtcategory, drtcomments, buttonstatus) {
      ...
      if (this.billid == '') {
        ...
        this.isLoading = true;
        this.$http
          .post(`https://mis.dragarwal.com/api-drtbills`, formData, {}).then(response => {
